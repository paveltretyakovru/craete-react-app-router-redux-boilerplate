...
var _api_stub = __webpack_require__(15);
...
var API = exports.API = {
    ...
    sendVote: function sendVote(vote) {
...
var requireContext = __webpack_require__(14);
...
var UserInfo = exports.UserInfo = {
    numDeclension: function numDeclension(titles) {
    ...
    date_DnumMtxt: function date_DnumMtxt(date_string) {
        var months = ["января", "февраля", "марта", "апреля", "мая", "июня", "июля", "августа", "сентября", "октября", "ноября", "декабря"];
        var date = new Date(date_string);
        return date.getDate() + ' ' + months[date.getMonth()];
    },

    mustache_lambda: function mustache_lambda(outsideRender) {
        return function () {
    ...
    render: function render(template, userdata) {
...
var _userinfo = __webpack_require__(11);
...
var userdataReq = _api.API.getInfo();
...
function initDOM(userdata) {
    if (userdata) {
        ...
        $('.scene--intro').after(_userinfo.UserInfo.render(template, userdata));
    } else {
        var template = $("#page_error_template").html();
        $('.wrapper').html(_mustache2.default.to_html(template));
    }
...
(function () {
    ...
    function DataContext(isPersistent) {
         ...
         */
        var _putDataSet = function _putDataSet(key, obj, opt_fireListeners) {
        ...
        var _removeDataSet = function _removeDataSet(key, opt_fireListeners) {
         ...
         */
        var _registerListener = function _registerListener(keys, callback, oneTimeListener, fireIfReady) {
         ...
         */
        var isDataReady = function isDataReady(keys) {
            if (keys['*']) {
                return true;
            }
         ...
         */
        var maybeFireListener = function maybeFireListener(listener, key) {
         ...
         * @private
         */
        var fireCallbacks = function fireCallbacks(keys) {
            if (typeof keys == 'string') {
                keys = [keys];
            }
            for (var i = 0; i < listeners.length; ++i) {
        ...
        return {
             ...
             * listener is fired.
             */
            registerListener: function registerListener(keys, callback) {
                _registerListener(keys, callback, false, true);
            },
             ...
             * @param {function(Array.<string>)} callback Function to call when a.
             */
            registerOneTimeListener_: function registerOneTimeListener_(keys, callback) {
                _registerListener(keys, callback, true, true);
            },
             ...
             * @param {function(Array.<string>)} callback Function to call when a.
             */
            registerDeferredListener_: function registerDeferredListener_(keys, callback) {
                _registerListener(keys, callback, false, false);
            },
             ...
             * @param {ResponseItem|Object} obj The data object.
             */
            putDataSet: function putDataSet(key, obj) {
                _putDataSet(key, obj, true);
            },
             ...
             * @param {string} key The key to associate with this object.
             */
            removeDataSet: function removeDataSet(key) {
                _removeDataSet(key, true);
            },
             ...
             * listeners.
             */
            putDataSets: function putDataSets(dataSets) {
                var keys = [];
                for (var key in dataSets) {
                    keys.push(key);
                    _putDataSet(key, dataSets[key], false);
                }
                fireCallbacks(keys);
            }
        };
...
com.rooxteam.deferred.timeout = function (delay, promise) {
    ...
    if (promise.promise && promise.resolve) {
        var cancelPromise = function cancelPromise() {
            promise.rejectWith(promise, args.slice(1));
        };
        var cancelTimeout = function cancelTimeout() {
            timeout.rejectWith(timeout, args.slice(1));
        };
...
(function (docCookies, $, sharedcontext, gadgetsConfig) {
    ...
    var utils = com.rooxteam.statistic.utils,
        ...
        parameters = null,
        getDescriptorPathFromWidgetUrls = function getDescriptorPathFromWidgetUrls(widgetUrls) {
    ...
    */
    utils.wrapUriToRedirected = function (url, el) {
        var redirectorUrl = gadgetsConfig && gadgetsConfig.get('com.rooxteam.config').redirector && gadgetsConfig.get('com.rooxteam.config').redirector.url;
    ...
    utils.isExternalClick = function (el, evt) {
        ...
        try {
            ...
            evt.which === 2 || evt.metaKey || evt.ctrlKey) &&
            // rebuild current href
            el.tagName.toLowerCase() === 'a' && el.target !== '_blank' && !!aHref.length && !!href.length && href !== window.location.href && href.indexOf('#') < 0;
        } catch (e) {}
        return isExternal;
    };
    ...
    utils.isSyncCredential = function (storageKey) {
        var result = false;
        if (com.rooxteam.config.statistic.CREDENTIALS_SYNC_ENABLED && com.rooxteam.config.statistic.CREDENTIALS_SYNC_MAP && _typeof(com.rooxteam.config.statistic.CREDENTIALS_SYNC_MAP) === 'object') {
            for (var cookieKey in com.rooxteam.config.statistic.CREDENTIALS_SYNC_MAP) {
                if (com.rooxteam.config.statistic.CREDENTIALS_SYNC_MAP.hasOwnProperty(cookieKey) && com.rooxteam.config.statistic.CREDENTIALS_SYNC_MAP[cookieKey] === storageKey) {
                    result = true;
                }
            }
        }
    ...
    utils.syncCredentialItem = function (cookieKey, storageKey) {
        var cookieItem,
            storageItem,
            logger = window.gadgets ? window.gadgets : utils.getConsole();
    ...
    utils.syncCredentials = function (needSync) {
        if (needSync && com.rooxteam.config.statistic.CREDENTIALS_SYNC_MAP && _typeof(com.rooxteam.config.statistic.CREDENTIALS_SYNC_MAP) === 'object') {
            for (var cookieKey in com.rooxteam.config.statistic.CREDENTIALS_SYNC_MAP) {
    ...
    utils.getPerformanceLoadTime = function () {
        if (window.performance && window.performance.timing && window.performance.timing.navigationStart && window.performance.timing.loadEventEnd) {
    ...
    utils.getPerformanceData = function () {
        ...
        if (window.performance && window.performance.timing && window.performance.timing.navigationStart) {
            for (var key in window.performance.timing) {
                ...
                count = 0;
                while (performanceTiming.hasOwnProperty(newKey + '' + count)) {
                    count++;
                }
...
})(com.rooxteam.docCookies, __webpack_provided_window_dot_jQuery, com.rooxteam.sharedcontext, window.gadgets && window.gadgets.config);
;
...
(function (gadgets, utils, console) {
    ...
    com.rooxteam.statistic.updateConfiguration = function (localConfig, overrideConfig, widgetName, pageName) {
        if (widgetName) {
            if (typeof overrideConfig.DOM_EVENTS_WIDGETS != "undefined" && typeof overrideConfig.DOM_EVENTS_WIDGETS[widgetName] != "undefined") {
            ...
            if (typeof overrideConfig.DOM_EVENTS_PAGES != "undefined" && typeof overrideConfig.DOM_EVENTS_PAGES[pageName] != "undefined") {
                $.extend(localConfig, overrideConfig, {
...
(function () {
    ...
    function StatisticDao(storage_statistic, init_context) {
        ...
        if (!utils.isIE() && config.USE_LS_WITH_MUTEX) {
            ...
            this.storage = {
                ...
                putDataSet: function putDataSet(key, name) {
    ...
    StatisticDao.prototype = {
        _init: function _init(init_context) {
        ...
        saveUpdateStatistic: function saveUpdateStatistic(report) {
        ...
        saveUpdateContextValue: function saveUpdateContextValue(key, value) {
        ...
        _initContext: function _initContext(init_context) {
            var context = this._getStorageItem(this.storage_statistic_context);
            if (!context) {
                context = {};
            }
        ...
        _saveUpdateStorageItem: function _saveUpdateStorageItem(name, item) {
            var data = this.storage.getDataSet(this.storageId);
            if (data) {
                data[name] = item;
                this.storage.putDataSet(this.storageId, data);
        ...
        _getStorageItem: function _getStorageItem(name) {
            var data = this.storage.getDataSet(this.storageId);
            if (data) {
                return data[name];
            } else {
...
var CryptoJS = CryptoJS || function (e, m) {
    var p = {},
        j = p.lib = {},
        l = function l() {},
        f = j.Base = { extend: function extend(a) {
        ...
        n = j.WordArray = f.extend({ init: function init(a, c) {
            ...
            var c = this.words,
                q = a.words,
                d = this.sigBytes;a = a.sigBytes;this.clamp();if (d % 4) for (var b = 0; b < a; b++) {
        ...
        b = p.enc = {},
        h = b.Hex = { stringify: function stringify(a) {
        ...
        g = b.Latin1 = { stringify: function stringify(a) {
            var c = a.words;a = a.sigBytes;for (var b = [], d = 0; d < a; d++) {
        ...
        r = b.Utf8 = { stringify: function stringify(a) {
            try {
        ...
        k = j.BufferedBlockAlgorithm = f.extend({ reset: function reset() {
            ...
            var c = this._data,
                ...
                }g = b.splice(0, a);c.sigBytes -= d;
            }return new n.init(g, d);
        }, clone: function clone() {
            var a = f.clone.call(this);
            a._data = this._data.clone();return a;
        }, _minBufferSize: 0 });j.Hasher = k.extend({ cfg: f.extend(), init: function init(a) {
...
(function () {
    var e = CryptoJS,
        ...
        j = m.Hasher,
        l = [],
        m = e.algo.SHA1 = j.extend({ _doReset: function _doReset() {
...
(function (CryptoJS, $, utils, console) {
    ...
    function obfuscateUrlParams(url) {
        var filters = com.rooxteam.config.statistic.LIC_OBFUSCATED_DATA,
            ...
            makeRegExp = function makeRegExp() {
            var regExpArr = [];
            ...
            replaceObfuscatedParam = function replaceObfuscatedParam(paramStr) {
            ...
            testParam = function testParam(re, param) {
    ...
    function deepDataFilter(o, level) {
        ...
        for (var i in o) {
            if (o.hasOwnProperty(i)) {
                if (Array.isArray(o)) {
                ...
                } else if (o[i] !== null && o[i] !== undefined && _typeof(o[i]) === 'object') {
                    deepDataFilter(o[i], ++level);
                    ...
                    if (typeof o[i] !== 'string' && typeof o[i] !== 'number') {
                        console.log('Attempt to obfuscate non string or number - ' + _typeof(o[i]));
                    }
    ...
    function deepFilter(content) {
        ...
        try {
            ...
            if (contentObj.data && _typeof(contentObj.data) !== 'object') {
...
})(window.CryptoJS, __webpack_provided_window_dot_jQuery, com.rooxteam.statistic.utils, com.rooxteam.statistic.utils.getConsole());
;
...
(function ($) {
    ...
    JoinStatisticTransport.prototype = {
        ...
        sendReport: function sendReport(content, parameters, successCallback, errorCallback) {
            var promises = [],
                self = this;
            $.each(transports, function (indexInArray, transport) {
        ...
        validate: function validate(content, parameters) {
            var isValid = true;
            $.each(transports, function (indexInArray, transport) {
...
(function ($, utils) {
    ...
    GetStatisticTransport.prototype = {
        ...
        isRcmLicUrl: function isRcmLicUrl() {
         ...
         */
        sendSyncReport: function sendSyncReport() {
            var args = Array.prototype.slice.call(arguments);
        ...
        filterReport: function filterReport(content) {
            var parameters;
            if (this.useFilters) {
        ...
        _composeUrl: function _composeUrl(content, parameters) {
        ...
        _createUpdateImage: function _createUpdateImage(imgSrc, onLoad, onError) {
...
})(__webpack_provided_window_dot_jQuery, com.rooxteam.statistic.utils);
;
...
(function ($, utils) {
    ...
    PostStatisticTransport.prototype = {
        ...
        sendReport: function sendReport(content, parameters, successCallback, errorCallback, isAsync, forceContent) {
            var serverAddressKey = this.serverAddressKey || 'SERVER_ADDRESS',
...
com.rooxteam.statistic.client = function (self, gadgets, console, $, utils) {
    ...
    self.trySendReport = function (stackSize) {
        ...
        if (statisticBuffer && statisticBuffer.length > 0) {
            ...
            if (self.isExpired(config.ACCUMULATE_TIME, bufferLastSend)) {
                ...
                if (isValid) {
                    statisticTransport.sendReport(report, parameters, function () {
    ...
    self.sendSyncReport = function (statEvents) {
        var getTransport = function getTransport(addressConfigKey) {
            var transport;
            if (!utils.hasCorsXHR()) {
        ...
        if (config.LIC_ENABLED) {
            ...
            return $.when(statisticTransport.sendSyncReport(report, parameters), statisticLicTransport.sendSyncReport(report, parameters));
        } else {
            return $.when(statisticTransport.sendSyncReport(report, parameters));
        }
    };
    ...
    */
    ...
    self.logOperationSync = function (name, data, quantity, timeStart, timeEnd, callback) {
        ...
        if (name) {
            ...
            this.sendSyncReport(statisticBufferSync.getStatistic()).done(function () {
                if (!called) {
    ...
    self._init();
    return self;
}(com.rooxteam.statistic.client || {}, window.gadgets, com.rooxteam.statistic.utils.getConsole(), __webpack_provided_window_dot_jQuery, com.rooxteam.statistic.utils);
...
(function (gadgets, $, WinJS, document, TraceKit) {
    ...
    com.rooxteam.statistic.registerWidgetsLoadHandlers = function () {
        ...
        var onLoadHandler = function onLoadHandler(mid) {
            return function () {
    ...
    */
    ...
    com.rooxteam.statistic.processEvent = function precessEvent(event) {
        var getParentOrThis = function getParentOrThis(el, sel) {
            var $el = $(el);
            if (!$el.is(sel)) {
        ...
        if (config.EXTERNAL_LINKS_LOG_ENABLED && ev.type === 'click' && com.rooxteam.statistic.utils.isExternalClick(target, ev)) {
            ev.preventDefault();
...
(function (gadgets, $, document) {
    ...
    (function () {
        var renderType = utils.getRenderTypeFromContainer(),
            setupVisibilityChangeListner = function setupVisibilityChangeListner() {
            $(document).on('visibilitychange', function () {
                notifyVisibilityChanged(renderType);
            });
        },
            widgetReadyNotifyDisplay = function widgetReadyNotifyDisplay(callback) {
            if (renderType === 'INLINE') {
                $(document).ready(function () {
                    notifyVisibilityChanged(renderType);
                    if (callback) {
    ...
    window[eventMethod](messageEvent, function (e) {
        ...
        if (message && message.id === 'widget.display') {
            ...
            if (config.VIEW_EVENTS_ENABLED === true) {
                ...
                var dataLog = com.rooxteam.statistic.getContext(widgetData, utils.getUserPrefs(widgetData['mid']), utils.compact({
...
})(window.gadgets, __webpack_provided_window_dot_jQuery, window.document);
;
(function (stat) {
...
var symbol = new __WEBPACK_IMPORTED_MODULE_0_svg_baker_runtime_browser_symbol___default.a({
  ...
  "use": "spr-mouse-usage",
  "viewBox": "0 0 26 38",
  "content": "<symbol viewBox=\"0 0 26 38\" id=\"spr-mouse\"><defs><path id=\"spr-mouse_a\" d=\"M949 1761.5a11.5 11.5 0 0 1 23 0v12a11.5 11.5 0 0 1-23 0zm1 12a10.5 10.5 0 1 0 21 0v-12a10.5 10.5 0 1 0-21 0z\" /><path id=\"spr-mouse_b\" d=\"M959 1760.5a1.5 1.5 0 1 1 3 0v3a1.5 1.5 0 1 1-3 0z\" /></defs><use fill=\"#fff\" xlink:href=\"#spr-mouse_a\" transform=\"translate(-947 -1748)\" /><use fill=\"#fff\" xlink:href=\"#spr-mouse_b\" transform=\"translate(-947 -1748)\" /></symbol>"
});
